<template>
  <div class="lb-table" ref="table" v-loading="loading">
    ...
    <el-pagination
      :style="{
        'margin-top': tablePoprs.paginationTop,
        'text-align': tablePoprs.paginationAlign,
      }"
      v-if="tablePoprs.pagination"
      v-bind="tablePoprs"
      :current-page.sync="tablePagination.currentPage"
      :page-size.sync="tablePagination.pageSize"
      :total="tablePoprs.total"
  ...
  methods: {
    ...
    async getTableData() {
      ...
      if (this.tablePoprs.api) {
        const result = this.tablePoprs.resultField
          ? data[this.tablePoprs.resultField]
          : data;
        if (this.tablePoprs.pagination) {
          const total = this.tablePoprs.totalField
            ? data[this.tablePoprs.totalField]
            : data;
          this.tablePoprs = {
            data: result,
            total: total,
